...
def main():
    ...
    parser.add_argument('--ic', dest='ini_con', metavar='months',
                        type=str, nargs=1,
                        help='Month of Initial condition ex. Jan Feb')
    parser.add_argument('--lead', dest='leadtime', metavar='number',
                        type=int, nargs=1,
                        help='Leadtime in months (number)')
    # Specify models to exclude from command line
    parser.add_argument('--no-cfs', dest='cfs_bool', action="store_true",
                        default=False,
                        help="Don't display CFS V2 information")
