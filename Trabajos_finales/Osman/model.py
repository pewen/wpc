...
def generate_figure():
    ...
    m = Basemap(projection='merc', llcrnrlat=-60, urcrnrlat=10,
                llcrnrlon=360-85, urcrnrlon=360-30, lat_ts=20, resolution='c')
    # Draw coastlines, etc.
    m.drawcoastlines()
    m.drawstates()
    m.drawcountries()
...
class CFSV2(Model):
    ...
    def download_files(self):
        ...
        # anonymous@ruta = 'ftp.cpc.ncep.noaa.gov'
        ftp.login()
        ...
        # ruta para archivo que se escribe
        local_filename = os.path.join('data', nombre_archivo)
        # abro el archivo donde se va a alojar
        file = open(local_filename, 'wb')
        print("Writing GRIB file")

        # recibo info y guardo en archivo
        ftp.retrbinary('RETR ' + nombre_archivo, file.write)
        file.close()       # cierro archivo
        # This is the “polite” way to close a connection  me voy del ftp
        ftp.quit()
